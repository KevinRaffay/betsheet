...
| `server/pl.js` | P/L reporting (**D175: the TIPSHEET bucket total counts ONE variant per (day, source)** - the three are mutually exclusive alternatives, only one of which is ever real money, and summing them reported ~$598 wagered on a $200 bankroll. The counted variant is FIXED (`TIP_HEADLINE_VARIANT`, `win-only`) because picking the best performer per day would be cherry-picking; the others ride along as `byVariant`, a breakdown never added to the total. This is Distributions' long-standing "one card per day per bucket" rule finally arriving in P/L.): `GET /api/pl` - per-completeness-bucket totals for ONE engine version (default the latest graded card's version, `all` pools on request), every graded card as a row, ungraded listed separately, deliberately NO pooled all-bucket total (invariant 13). `GET /api/race-days/:id/pl` - per-race × per-card breakdown. The LLM_GENERATED bucket entry also carries `byModel` (a breakdown by `cards.llm_model`, never a second pool) and `byNotes` (the same rows grouped by `cards.notes_present`, notes-first, deterministic order). |
...
| `server/llm-prompt.js` | LLM card prompt, PURE. Mirrors `docs/prompts/llm-card-v1.md`. **D179: a BASELINE PICKS block** - the day's tip sheets (ranked) and OTR (printed TICKETS, its own vocabulary, D74's reason: it never claims a ranked 3rd pick), LABELLED per source, both when both exist. Resurrected from the CONSENSUS block D112 deleted (`git show 2543e52`). `BASELINE_CLAUSES` appends to the system prompt ONLY when a baseline exists, the same conditional shape the notes clauses use - **an absent baseline renders no block and `buildSystemPrompt({})` is still `SYSTEM_PROMPT`**. Source labels are free text and pass `sanitizeSourceLabel`. **D178 removed three DEAD inputs**: the opening promised program handicapper rankings, the program's Bottom Line, and consensus picks - consensus was deleted by D112, the other two by D113's removal of the Del Mar program ingestion, and **0 of 574 entries / 0 of 49 races on an ACTIVE day carry any of them**. A system prompt promising inputs that never arrive describes a different task than the one being asked; the token saving (159 chars, ~40 tokens, 1.9%) was the smaller reason. The rest of SYSTEM_PROMPT is load-bearing (D145/D146/D148/D160-D163) and must not be cut. `buildSystemPrompt({hasNotes})` and its `ANALYST_NOTES_CLAUSES` **must stay CONDITIONAL** - `buildSystemPrompt({hasNotes:false}) === SYSTEM_PROMPT` is asserted, since an LLM card has NO version axis (`engine_version` is the literal `'llm'`) and an unconditional prompt change would pool silently with every prior card. Every real prompt-content change is therefore an un-versioned, permanent boundary, checkable per card via `llm_card_requests.prompt_text` for a phrase that predates the change (the codebase's PR history is a long list of these; see DELIVERABLES.md for the individual fixes). `sanitizeNotesForPrompt` destroys marker-shaped text at the INPUT boundary. `extractTicketBlock` splits a response on the `<<<TICKETS>>>` markers, `null` when missing (hard failure). `PROMPT_TEMPLATE_ID`/`PROMPT_TEMPLATE_VERSION` identify which template rendered a stored prompt - the version is a hash of `SYSTEM_PROMPT` computed at module load, not a manually bumped counter. |
| `server/llm-notes.js` | analyst notes (**D167: `writeNote` normalizes `source_label` on the way IN**, via `shared/source-labels.js`, so `llm_notes` carries the guarantee `tip_picks` does and grouping is exact. The label REACHES THE PROMPT and LLM cards have no version axis, but this makes prompts MORE comparable rather than less - `Program` and `program` used to render as two different prompts for one source - and it is provably INERT for every value in the real corpus, asserted with a negative control. An empty label still stores NULL; the `user` fallback is applied at READ time, as always.): the mutable DRAFT store plus the composition that turns a draft into the immutable per-call snapshot. Keyed by `(race_day_id, race_number)`, 0 = the day-level note - NOT by `card_id`, since notes belong to a RACE and the first preview of a brand-new card has no card id yet. `llm_notes` is the only mutable table in the LLM subsystem; the immutable record is the snapshot on `llm_card_requests`. `loadNotesForRace` also returns the composed/sanitized/truncated text it hashes, so a caller storing it doesn't recompute it. |
...
| `client/src/components/RaceDayNotesModal.jsx` | D159, narrowed by D184: the **WHOLE-DAY** analyst note (race 0) and nothing else - per-race notes are typed in each race's own panel under the house rule in Gotchas. A second entry point onto the SAME `llm_notes` draft (D92) `LlmCardModal.jsx` reads/writes, reachable from `RaceDayView.jsx` before any card exists. No new endpoint and no schema change: `GET/PUT /api/race-days/:id/llm-notes`, keyed by `(race_day_id, race_number)` as always, so a note entered here is the same row the generator shows. D184 also dropped its `getRaceDay` call - it no longer lists the day's races, so it no longer needs the day. Flushes any pending edit on Close/Escape before calling `onClose`, matching the modal-backdrop house rule (no backdrop-click dismissal). |
...
| `shared/source-labels.js` | D167: the ONE place this codebase answers **"who said this"**. PURE and browser-safe, imported by the server (`llm-notes`, `tip-extraction`) AND the client (`AnalystNotesEditor`) - which is the point, since the notes vocabulary used to live in a `.jsx` where the server could not see or enforce it. Owns the single `normalizeSourceLabel` (one lowercase slug; an unknown name keeps its OWN slug rather than being refused, so it stays countable and promotable) and `isNormalizedSourceLabel`, the inertness probe. **The two catalogues are NOT merged, deliberately**: `NOTE_SOURCE_LABELS` answers what KIND of commentary a note is (`program`, `public-handicapper`, `llm`, `own`), `TIP_SOURCE_LABELS` answers WHICH APP published a pick set (`trackmaster`, `numberfire`) - `trackmaster` BELONGS TO the kind `public-handicapper`, so they are two axes, not two halves of one. **The two fallbacks stay distinct too**: a blank note is the USER's (`user`), while an unreadable tip sheet is not anyone's personal opinion (`tipsheet-other`) - collapsing them would mislabel a failed screenshot as the user's own view. |
...
| `server/tip-picks.js` | D169: the TIPSHEET HTTP surface and the human-correction path. **D176 adds `POST .../tip-picks/manual`** - picks typed from the printed sheet, no vision call. It produces the IDENTICAL payload extraction does, through the same `validateTipPicks` and the same `insertTipPicks`, so scoring and staking cannot tell the two apart; `raw_extraction`/`model`/`image_sha256` simply stay NULL, which is the honest record and makes provenance readable without a flag. Horse NAMES come from the day's own entries, never the client. **No odds are captured** - the morning line already sits on `entries`, and a tip sheet's printed price is a different number that copying the ML would invent. A sheet with no picks DELETES that source's row, the same clear-by-emptying shape `writeNote` uses. `POST .../tip-picks/preview` calls the model, ARCHIVES the image and the response under `BETSHEET_TIP_ARCHIVE_DIR` regardless of outcome, and **persists nothing**; `POST .../tip-picks` re-reads the ARCHIVED RESPONSE and re-parses it server-side, so a client cannot inject picks (asserted). **The archived artifact is the model's RESPONSE, not just the image** - unlike `equibase-otr.js`, confirm cannot re-derive by re-parsing the source, because re-extraction is a paid, non-deterministic call. `PATCH /api/tip-picks/:id` is the CORRECTION path: it validates through the same `validateTipPicks` the model's output passes, writes `picks_extracted` on the FIRST edit only (so it always holds the model's original, never the previous edit) and stamps `edited_at`. **Invariant 9 is intact, not excepted** - the preview is read-only and a save stores verbatim model output; correcting is a separate, later, recorded act. `BETSHEET_TIP_TEST_MODE=1` + `__stubResponse` is the check-only bypass. |
| `client/src/components/TipPicksEntryModal.jsx` | D176: manual tip-pick entry for ONE race. The grid is horse ROWS by tipsheet COLUMNS, **each cell three toggle buttons 1/2/3** (D185; a rank `<select>` until then) - **not one tipsheet dropdown per horse**, because a horse routinely appears in several sheets at different ranks (in the two real screenshots this project extracted, two of the three horses did) and a per-row sheet picker cannot express that at all. Clicking a lit button CLEARS that rank; clicking any other MOVES the rank off whichever horse held it in that column, so a sheet cannot rank two horses first - one `setRank` rule, reused rather than copied, and the server validates it again through the same `validateTipPicks` a model's output passes. The lit state is plain `.btn--primary` and carries `aria-pressed`, so the control reports the same thing to a screen reader that the fill reports to the eye. Every column is submitted on save including empty ones - **an empty sheet is how a column is cleared**, and omitting it would leave stale picks. Opens seeded from the race's existing rows, so it edits rather than starting blank. Modal-backdrop house rule: no click handler. |
| `client/src/components/RaceTipPicks.jsx` | D182: ONE race's tip sheets, rendered inside that race's own `<details>` on `RaceDayView.jsx` - the same shape `RaceNotes.jsx` has for analyst notes, and for the same reason: a tip sheet is an opinion about a RACE. Owns the picks table, the score line, the D169 correction editor and delete, and opens D176's entry dialog. **The two surfaces stay deliberately separate** (invariant 9): a save stores what was entered, and CORRECTING is a separate, later, recorded act on a saved row (`picks_extracted` keeps the first version, `edited_at` stamps it). Several sheets can rank one race, so each source gets its own `.tip-source` block rather than a run-on table. Takes `rows`/`scoreFor` as PROPS and fetches nothing - `RaceDayView` loads the day's rows and scores once, so twelve races cost one request, not twelve. `onChanged` refetches the day, because the staking panel's source list is derived from the same rows. |
| `client/src/components/TipStakingPanel.jsx` | D171/D174, relocated by D182, ONE button since D183: turn the day's tip sheets into three comparable cards per source. **This is the ONE part of the tipsheet feature that is not per-race, and it is structural, not a layout preference**: `planTipCards` sizes every ticket from `perRaceBankrollCents(bankroll, racesWithPicks.length)`, so race 1's ticket price depends on how many OTHER races have picks and a fourth race re-prices the first three. A per-race "stake this race" button would have to invent a denominator. **D183's single "Stake all tip sheets into cards" is a CONVENIENCE, NEVER A MERGE** - each source is still previewed and saved through its own per-source call and still gets its own three cards carrying its own `tip_source_label`, because pooling two sources would break invariant 13 outright (whether TrackMaster agrees with NumberFire is the thing being measured, so a card that cannot say which sheet picked it answers nothing). Sources are saved SEQUENTIALLY and **each outcome is reported by name** - one source failing neither stops nor hides the others, and D174 makes the remedy "press it again". **No total is shown across variants or sources**: the three variants are mutually exclusive ways to bet the same picks, and summing them is exactly the D175 bug that reported ~$598 wagered against a $200 bankroll. Takes `rows` as a prop rather than fetching them, so it cannot disagree with the race panels about which sources exist. **`onSaved` remounts the sibling `CardsPanel`** (D173) - it self-fetches on mount, so without that wire staked cards appear only after a page reload. |
...
| `client/src/components/RaceNotes.jsx` | D164/D165: the READ-ONLY "Analyst Notes" collapsible, `RaceNotes({ note })` - one race's `{text, sourceLabel, updatedAt}` or none. No fetch of its own. **D184 narrowed it to `CardView.jsx` alone**: `/day` now EDITS a race's note in place (`RaceNotesEditor.jsx`), while a card sheet is a record of what was decided and must not offer to change it. The two are deliberately separate components rather than one with an `editable` flag - the read-only case has no day id, no save path and no dirty state, and giving it those to serve one caller is how a record surface quietly becomes an editor. |
| `client/src/components/RaceNotesEditor.jsx` | D184: ONE race's analyst notes, EDITED in that race's own panel on `RaceDayView.jsx` - the house rule that a race-specific input belongs in the Race UI component (see Gotchas). Reuses `NotesEditor` from `AnalystNotesEditor.jsx` rather than forking it, and writes the same `llm_notes` draft (D92) the LLM generator reads - same endpoint, same row, no schema change. Flushes on blur, matching every other notes surface (there is no Save button in this feature). **An unsaved local edit always beats an arriving refetch**, and the prop is adopted during RENDER rather than in an effect (D155). `postResult` is carried down so the not-blind warning appears where a note is actually typed. Takes `note` as a prop: `RaceDayView` loads every race's note in ONE call. |
...
| `client/src/components/RaceDayView.jsx` | read-only view of a stored day (**`cardsVersion` is bumped by EVERY sibling that writes a card** - the OTR upload (D142) and tip staking (D173). A new sibling that writes cards needs the same `onSaved` wire or its cards need a page reload to appear.) - what landed in the DB, not what the parser proposed. A day-level staleness banner plus a per-race tag (`past post`/`entries Nm old`), ONE `now` for the whole render so banner and tags agree. A day with no `odds_captured_at` renders no banner. Holds a `cardsVersion` counter passed as `CardsPanel`'s `key` (remount = reload), bumped by `EquibaseOtrPanel`'s `onSaved` callback. **"Enter Analyst Notes" (D159)** opens `RaceDayNotesModal.jsx`, reachable the moment entries are in, before any card exists. **D164/D182**: each race's `<details>` carries two nested, collapsed-by-default panels - the read-only "Analyst Notes" panel (`notesByRace`, fetched via the same `GET /api/race-days/:id/llm-notes` the modal uses; no editing here, and closing the modal refetches so an edit shows up without a page reload) and `RaceTipPicks.jsx` (**D182**: that race's tip sheets, entered and corrected in place). **The day's tip rows and their scores are loaded ONCE here** and passed down to every race panel AND to the day-level `TipStakingPanel` - one request for the day rather than one per race, and no second copy that could disagree about which sources exist. |
...
| Tip sheets moved into the race UI they describe (D182) | merged | PR [#224](https://github.com/KevinRaffay/betsheet/pull/224), branch `tipsheets-into-race-ui` - **user request**: a tip sheet is tightly coupled to a race, like analyst notes, so it should be managed in the race UI rather than a day-level list. Each race's `<details>` now carries its own `Tip sheets` panel (`RaceTipPicks.jsx`) beside `RaceNotes.jsx` - enter, review, correct, delete, and the race's score line, all in place. **STAKING did not move, and could not**: `planTipCards` sizes every ticket from the bankroll split across every race that has picks, so race 1's price depends on how many other races have picks; a per-race stake button would have to invent a denominator. It stays day-level as `TipStakingPanel.jsx`, renamed to say what it now does. The day loads tip rows and scores ONCE and passes them down, replacing a duplicate `listTipPicks` call and removing a remount key in the process. Full record: DELIVERABLES.md D182. |
| A program-less scratch is stored NULL, not 'SCR' (D180) | merged | PR [#220](https://github.com/KevinRaffay/betsheet/pull/220), branch `fix-scr-program-numbers` - **the investigation corrected the bug report, including my own from D178**. It is not a parser defect: `shared/parsers/equibase-entries.js` reads those rows correctly and returns `programNumber: null`. The placeholder came from `program_number TEXT NOT NULL`, and the number is genuinely absent from the page - verified in the saved Equibase capture, where a scratch row is `<tr class="scratch"><td colspan="2">SCR</td>`, the number and post-position cells replaced. **And it had corrupted nothing**: re-measured against the SELECTIONS column alone, 0 of 3,156 program references across the whole corpus were wrong. Migration 032 makes the column nullable and backfills 150 rows; SQLite's UNIQUE permits many NULLs, so D122's suffix hack is gone rather than kept. Found and fixed a real hole on the way: `resolveToken`'s name-only path would have returned a NULL number, and the scratch guard keys on the number - the one way a scratched horse could still have been bet. Full record: DELIVERABLES.md D180. |
| Tipsheet and OTR picks as a baseline LLM input (D179) | merged | PR [#219](https://github.com/KevinRaffay/betsheet/pull/219), branch `llm-baseline-inputs` - the LLM generator now sees what the day's other sources think, labelled per source, beside the optional notes. **Archaeology found the work half-done in history**: D112's deleted CONSENSUS block already solved the shape, including D74's reason for rendering OTR as tickets rather than ranks. It also found the heavier half of the original scope unnecessary - `llm_card_requests.prompt_text` already stores the whole user prompt verbatim, so there is NO migration, no columns and no latch. Scope committed as `docs/requirements/llm-baseline-inputs.md` before the code. **Records its own consequence**: LLM cards are now derivative of the EQB_OTR and TIPSHEET buckets, so comparing them no longer measures what it did - acceptable only because the corpus is a sandbox. Full record: DELIVERABLES.md D179. |
| Three dead inputs removed from the LLM prompt (D178) | merged | PR [#218](https://github.com/KevinRaffay/betsheet/pull/218), branch `llm-prompt-trim-dead-fields` - **arrived as a different proposal**: strip program NUMBERS from the prompt to stop the model "thrashing" and cut cost. Measured against the corpus first, and the premise did not hold - across 257 responses and **3,190 program-number references, 4 were wrong (0.1%)**, and both failing responses INVERT on inspection: the model named the correct horses (#3, #8) and the stored entries had lost their numbers to a `SCR`/`SCR-2` parser defect. Numbers also cost only ~2.5% of the user prompt, and removing them would delete the number-vs-name cross-check and turn a non-blocking `name_mismatch` into a blocking `unknown_program`. What WAS dead: program rank, BEST BET, the Bottom Line and a system-prompt promise of consensus picks deleted back in D112. Full record: DELIVERABLES.md D178. |
| The tip-sheet screenshot path removed (D177) | merged | PR [#217](https://github.com/KevinRaffay/betsheet/pull/217), branch `remove-tipsheet-extraction` - **user decision**, once D176's typed entry proved out: extraction was slow and cost an API call per race for three horses and three ranks. Deleted `server/tip-extraction.js` (the vision call, prompt, JSON recovery, magic-byte sniffing), the preview and parseToken-confirm routes, the image archive, `BETSHEET_TIP_TEST_MODE`, the upload UI and the sample screenshots. **The writer and reader survived, relocated into `server/tip-picks.js`** - they were never part of the vision half. The `raw_extraction`/`model`/`image_sha256` COLUMNS also survive: rows extracted before this carry real values, and a shipped migration is immutable regardless. Nothing downstream changed - scoring, staking, P/L and the validator were all untouched, which is the evidence the layering was right. Full record: DELIVERABLES.md D177. |
| Manual tip-pick entry: type the sheet instead of photographing it (D176) | merged | PR [#216](https://github.com/KevinRaffay/betsheet/pull/216), branch `tipsheet-manual-entry` - **user request**: screenshots are tedious and cost an API call per race, when a tip sheet is three horses and three ranks. A per-race dialog, horse rows x tipsheet columns, writing the IDENTICAL payload extraction produces. **The grid shape came from the user's own data**: the described one-tipsheet-dropdown-per-row could not record Karazest as TrackMaster #1 AND NumberFire #2, which both real screenshots showed - Kevin's call was columns per sheet. Extraction is untouched here and removed separately (D177), so the new path proved out before the old one went. Full record: DELIVERABLES.md D176. |
| TIPSHEET P/L counted three mutually-exclusive variants as one spend; plus P/L on the day view (D175) | merged | PR [#215](https://github.com/KevinRaffay/betsheet/pull/215), branch `tip-pl-variant-and-day-view` - **found while answering a user question about merging the three cards.** `server/pl.js` sums every card in a bucket, so the three variants - three ways to bet the SAME picks, only one of which is ever real money - reported ~$598 wagered on a $200 bankroll (an ROI-on-bankroll of -299%). Distributions never had this: D20 has always taken ONE card per day per bucket. The total now counts one variant per (day, source), FIXED as `win-only` because picking the best performer per day would be cherry-picking, with `byVariant` beside it as a breakdown that is never added in. Second half, same PR because it is the same question ("what did this card do?"): the day view's Betting cards table gained a **P/L column** for graded cards, read from `graded_tickets_latest` - the same view `/api/pl` uses, asserted to agree to the cent. An ungraded card shows a dash, never $0. `plMoney`/`plClass` moved into `client/src/api.js` so `PLView` and `CardsPanel` cannot drift. Full record: DELIVERABLES.md D175. |
| One tipsheet card per source, growing as races are added (D174) | merged | PR [#214](https://github.com/KevinRaffay/betsheet/pull/214), branch `tipsheet-one-card-per-source` - **user request**: a tip sheet arrives race by race, and D171's append-only rule turned that into three MORE cards on every stake. Now ONE card per (day, source, variant), found via `cards.tip_source_label` (migration 031, backfilled from the name D171 wrote), reused and **recomputed whole** - required, not convenient, because the per-race budget is the bankroll split across races-with-picks, so a new race re-prices the old ones. Matches how `human-cards.js`/`llm-cards.js` already accumulate races onto one card. Re-staking a graded card discards its grades and regrades - warned, never blocked (D149/D140). `npm run dedupe-tip-cards` folds pre-existing duplicates, refusing graded cards. Full record: DELIVERABLES.md D174. |
...
| TIPSHEET staking: tip picks become graded cards, in three comparable variants (D171) | merged | PR [#209](https://github.com/KevinRaffay/betsheet/pull/209), branch `tip-picks-staking` - closes the tipsheet arc D166-D170. **The user supplied a drafted `shared/tipStaking.js`; archaeology confirmed it existed nowhere in the repo or its history, and running it proved it could not work here**: its input is an ML-vs-LIVE-odds gap, and `entries.live_odds` is filled on 0 of 13,710 rows. Against both real screenshots it produced ZERO bets and zero exclusions - silently empty, since every NaN comparison is false. Its one rule that works from the morning line alone (place money at 8/1+) was kept **and corrected**: its threshold of 9 assumed European decimal odds, while this codebase's `morningLineToDecimal` returns the fractional ratio, so it would have skipped 8/1 exactly. Migration 030 rebuilds `cards` (verified against the REAL corpus: 136 cards, 3589 tickets, 3042 graded, every count identical). Full record: DELIVERABLES.md D171. |
| TIPSHEET scoring: is a tip source worth reading at all? (D170) | merged | PR [#208](https://github.com/KevinRaffay/betsheet/pull/208), branch `tip-picks-scoring` - **user decision 2026-09-08**, closing the loop D166/D169 left open: `tip_picks` had no consumer, so extraction produced data nothing read. Rank-based and **money-free** - nothing stakes a tip sheet yet, so a P/L figure would be invented. Judgement calls written into the code: a scratched top pick promotes the next live one (what a bettor would back), an unplaced also-ran is a loss but an unknown program number is SURFACED as a likely extraction misread, and anything unscorable returns NULL rather than zero. Every rate carries its `n` and is NULL at n=0, enforced in the UI too. **Not yet verified against a real multi-day corpus** - see the D170 ledger row. Full record: DELIVERABLES.md D170. |
...
| Review and correct extracted tip picks: the TIPSHEET HTTP surface and its edit path (D169) | merged | PR [#207](https://github.com/KevinRaffay/betsheet/pull/207), branch `tip-picks-review-ui` - the review/edit UI D166 explicitly deferred. D166 had left NO routes at all. **The design question was invariant 9**, which forbids hand-editing a parser's output and says corrections happen at the source - but a screenshot HAS no source to fix, and one misread program number silently scores a tipsheet on picks it never made. **User decision 2026-09-08: keep invariant 9 literally true rather than carve an exception** - it governs the parse->save path, which stays read-only, and correcting is a separate recorded act on a stored row (migration 029: `picks_extracted` + `edited_at`). Browser verification found a real defect: a `4/5` placeholder in an odds column reads as data. Full record: DELIVERABLES.md D169. |
