...
| Findings: lean-1.1 on the PROGRAM_ONLY corpus (D52) | in review | PR #58, branch `findings-lean-1-1` (stacked on #57) — docs/findings/lean-1.1-program-only.md from runs #27-#50 (every template, both scratch modes, all meets and per meet, paired day counts); the "Findings" section above with the per-version rule; no code |
