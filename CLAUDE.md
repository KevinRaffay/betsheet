...
13. **Completeness buckets never pool.** Every card records a
    ...
    human's own pasted tickets, never engine-generated or simulated) from
    the sources that actually contributed. All P/L, simulation, and
    distribution reporting buckets by this level — a program-only backfill
    card and a full-consensus card never share an aggregate, and a HUMAN
    card never shares one with any engine bucket. Structure-layer rules
