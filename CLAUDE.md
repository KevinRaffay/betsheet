...
| Human ticket parser: box selections accept '/' as well as ',' (D56) | in review | PR #72, branch `fix-box-selection-separator` — bug fix to D54, found live in Replay: `Exacta Box #1 / #6 $25` was read as ONE selection token (box types only split on ','), producing a spurious name-mismatch and a blocking "needs more horses". Box selections have no positional meaning, so '/' and ',' are now equivalent separators there; `npm run check-human-picks` grew a regression case |
