...
| feature | state | notes |
| --- | --- | --- |
| Tip sheets moved into the race UI they describe (D182) | in review | branch `tipsheets-into-race-ui` - **user request**: a tip sheet is tightly coupled to a race, like analyst notes, so it should be managed in the race UI rather than a day-level list. Each race's `<details>` now carries its own `Tip sheets` panel (`RaceTipPicks.jsx`) beside `RaceNotes.jsx` - enter, review, correct, delete, and the race's score line, all in place. **STAKING did not move, and could not**: `planTipCards` sizes every ticket from the bankroll split across every race that has picks, so race 1's price depends on how many other races have picks; a per-race stake button would have to invent a denominator. It stays day-level as `TipStakingPanel.jsx`, renamed to say what it now does. The day loads tip rows and scores ONCE and passes them down, replacing a duplicate `listTipPicks` call and removing a remount key in the process. Full record: DELIVERABLES.md D182. |
