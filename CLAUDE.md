...
| `server/tip-staking.js` | D171: `POST /race-days/:id/tip-cards/preview` (writes nothing) and `POST .../tip-cards` (three cards, ONE transaction, append-only - a second run mints three new cards rather than rewriting graded ones). Bucket TIPSHEET, `engine_version` 'tipsheet'. Per-race budget via D163's exported `perRaceBankrollCents` (whole-dollar floor), reused not reimplemented. Stores `selections` as `{races, legs}` - the shape `server/grading.js` reads back. |
