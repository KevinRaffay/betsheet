...
npm run check-equibase-apify-results # D193: the second RESULTS source vs. a real 11-race single-track Apify dataset (golden + hand counts; the two finalTime-less races; exotics-on-winner's-row-only; scratch derivation from a supplied entries context; apifyResultsToPayload's deliberate throw)
