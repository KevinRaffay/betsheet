...
| `server/ingest.js` | the ingest API: `POST /api/parse/entries-text`, `POST /api/parse/program-pdf` (raw `application/pdf` body, no multipart dep), `POST /api/race-days` (validated, transactional, 409-on-duplicate with explicit replace; all canonical matches, including soft-deleted tombstones, are superseded before insert), `GET /api/race-days[/:id]` (+`?deleted=1`), `GET .../deletion-preview`, `DELETE /race-days/:id` (soft, migration 005, logged to decision-trace), `POST .../restore`. Preview-first: parse endpoints never write. |
...
| Deleted race-day re-ingest collision fix (D50-QA) | merged | QA `qa` branch deliverable — re-ingest supersedes all canonical track/date matches, including legacy deleted tombstones, before inserting; validated with `npm run check-ingest` |
