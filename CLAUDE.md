...
| `client/src/components/TipPicksPanel.jsx` | D169: the tip-sheet panel on `RaceDayView.jsx` (**D173: takes an `onSaved` prop** - staking writes THREE cards into the sibling `CardsPanel`, which self-fetches on mount, so without that wire they appear only after a page reload; the same defect D142 fixed for the OTR panel) - upload a screenshot, review, save, correct. **Two deliberately separate surfaces**: the PREVIEW is read-only and offers no way to change anything, so a save is always verbatim; the CORRECTION editor acts on an already-saved row and shows "What the model originally read" beside it. Odds boxes are left EMPTY to mean "the sheet showed no price" - they carry no placeholder, because a placeholder like `4/5` in an odds column reads as data (found in the browser: all three rows appeared to carry a morning line). |
