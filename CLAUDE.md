...
- **A wager menu's minimum is only as good as the money token the parser can
  ...
  **The forms in this corpus are more varied than they look**: `$1`, `$.50`,
  `50c`, `20 Cent`, `10 cent`, `.50`, `0.20`, plus `Exactor`/`Triactor` for the
  exacta/trifecta at Canadian tracks. When a new track's menu arrives, check
  `parseWagerMenu` against it rather than assuming - and note the two shapes
  still unread, both harmless today only because they coincide with the
  fallback: a trailing parenthetical (`Trifecta (.50)`, `Super (.10)`) and an
  amount separated from its bet type by a word (`$3 PRIME PICK 3`).
