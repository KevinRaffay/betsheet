...
| SimView compare follow-up (D51) | in review | branch `sim-compare-view` (stacked on #56) — meet selector, vs lean column with paired day counts, drawdown deltas, the Simulated templates section on Distributions; read-side only, no schema change, ENGINE_VERSION lean-1.1; `npm run check-sim` +8 |
