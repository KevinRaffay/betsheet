...
npm run check-getascraper-entries # D219: the third entries parser vs. the real getascraper capture (golden + hand counts from the RAW file; the post-time timezone suffix stripped and cross-checked; the AM/PM regression guard; program numbers derived from post position, proven against the source's own exotic combinations; every refusal incl. the older snake_case shape refused by name) AND the calendar placement that justifies it - re-measuring parseforge's 0-of-11
npm run check-getascraper-ingest  # D219: that path end to end over real HTTP on a temp DB - preview writes nothing, migration 034 accepts the provenance value, post time persists, and the saved day comes back from GET /api/calendar with its race PLACED (8:30 AM PDT) rather than unplaceable
...
| feature | state | notes |
| --- | --- | --- |
| Third entries source: `getascraper/equibase-us-horse-racing-scraper`, entries only (D219) | in review | branch `getascraper-entries-parser` - **user request 2026-09-10**, scoped down from "add this actor to the UI" to entries-only after all three supplied captures were analyzed first. **The measured reason it exists**: `parseforge` carries NO post time (its own `fieldsNotProvided` says so; all 11 races of its Del Mar fixture are null), and `server/race-calendar.js` routes a race with no parseable post time into `unplaceable` - so every parseforge-sourced day is BLANK on the D209-D211 calendar, re-measured here as 0 of 11 placed. This source prints one per race with a timezone. **Results deliberately refused**: its win/place/show are the WINNER's three numbers alone (monotonic across all 22 races of two captures; a 4/5 winner paying $2.20 place while the 2nd horse was 12/1 settles it), and `shared/grading.js:66-67` scores an in-the-money horse with no price on file as `('loss', 0)` - so ingesting them would write real winnings into P/L as losses. Its multi-race exotics are also measurably corrupt (7/24 and 7/32 implausible). **Two load-bearing findings, both from measurement**: `postTimeMinutes('11:30 AM ET')` is null, so the suffix is stripped (and cross-checked against the track registry's IANA zone rather than discarded); and the strip's own first draft read `"AM"` as a zone, a silent 12-hour error caught by testing and now pinned by regression assertions - both written up in Gotchas. **No `programNumber` exists on this source**, so one is derived from `postPosition`, warned every time, and verified sound against the source's own exotic combinations (exacta `1-7` -> the 1st and 2nd finishers) - it diverges only on coupled entries, which no sample has. **A real regression, stated not buried**: no `distance` and no `surface` at all, plus no medication/age-sex/claim-price/wager-menu/scratch-flag - the price of the post times, and why this is NOT the default parser. Migration 034 adds a DISTINCT `equibase_getascraper` provenance value rather than reusing `equibase_apify`, because both actors are in use at once and differ in what they carry (invariant 13's provenance question). **The UI path is a file upload, not a live pull**: the actor's real input schema could not be read from any primary source (apify.com is blocked by this environment's egress proxy) and D197's precedent is that guessing one produces a confident wrong billed call - the export is verified real, needs no schema, and matches invariant 6's own posture. Full record: DELIVERABLES.md D219. |
| **Calendar and `/day` refinements, 2026-09-10 (D213-D218)** - twelve merged PRs (#264-#275) recorded retroactively | merged | Six deliverables, grouped by feature rather than one row per PR because several were 1-file increments to the same behaviour minutes apart. **D214** (#265/#266/#268): the calendar grid starts at 7 AM PT instead of 10, the track column is sticky under horizontal scroll, and the view auto-scrolls to the current Pacific hour on load. **D215** (#267/#269/#270/#271): every race carries a bookmarkable `#race-N` anchor, and arriving on one from the calendar collapses every other race and opens the target's analyst-notes and tip-sheets panels. **D216** (#272/#273): graded results render per race on `/day` via a new `RaceResults.jsx`, plus the camelCase/snake_case bug that shipped with it - this endpoint's PREVIEW shape is camelCase and its SAVED shape snake_case, and both are rendered in `ResultsPanel.jsx`, so copying the wrong half compiles and passes build. **D217** (#274): tip-sheet picks default to NumberFire then TrackMaster. **D218** (#275): calendar race links became buttons, green when the day has any card - **day-level, not per-race**, which is narrower than the request and recorded as such. **D213** (#264): a ledger tidy marking D209-D211 merged. **These rows are RETROACTIVE**, reconstructed from PR bodies and diffstats after merge, so each states what its PR actually recorded and says plainly where no verification evidence exists - the drift that made them necessary is itself why D213 exists. Full record per ID: DELIVERABLES.md D213-D218. |
...
- **A post time with a timezone suffix is UNREADABLE to the calendar, and
  `[A-Z]{2,4}$` eats the meridiem** (D219). `postTimeMinutes`
  (`shared/staleness.js`) returns **null** for `"11:30 AM ET"` - measured, not
  assumed - so a parser that stores a source's post time verbatim leaves every
  race in `server/race-calendar.js`'s `unplaceable` list while the `post_time`
  column looks populated, which is the worst shape a gap can take. Strip the
  suffix to `"11:30 AM"`, the form both other parsers already store. **And when
  you write that strip, exclude AM/PM explicitly**: `/\s+([A-Z]{2,4})$/` matches
  `"AM"`, so a bare `"11:30 AM"` parses as time `"11:30"` plus zone `"AM"` - a
  silent 12-hour error that shipped in `equibase-getascraper.js`'s own first
  draft and was caught by running the parser over a bare time, not by reading
  it. The printed zone is worth CROSS-CHECKING against the track registry's
  IANA zone rather than discarding (a disagreement means one of the two is
  wrong); derive the expected abbreviation with `Intl`, never a hardcoded table.
