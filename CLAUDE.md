...
| `client/src/components/TipPicksEntryModal.jsx` | D176: manual tip-pick entry for ONE race. The grid is horse ROWS by tipsheet COLUMNS, each cell a rank dropdown - **not one tipsheet dropdown per horse**, because a horse routinely appears in several sheets at different ranks (in the two real screenshots this project extracted, two of the three horses did) and a per-row sheet picker cannot express that at all. Assigning a rank MOVES it off whatever horse held it in that column, so a sheet cannot rank two horses first. Every column is submitted on save including empty ones - **an empty sheet is how a column is cleared**, and omitting it would leave stale picks. Opens seeded from the race's existing rows, so it edits rather than starting blank. Modal-backdrop house rule: no click handler. |
