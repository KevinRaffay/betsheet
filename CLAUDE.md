...
| `server/tip-extraction.js` | TIPSHEET extraction (D166): a screenshot -> validated picks -> optionally a `tip_picks` row. Vision call with **temperature OMITTED, not pinned** (claude-sonnet-5 rejects a non-default temperature with a 400 - see Gotchas; determinism is simply not on offer, though 3 repeated reads of each sample agreed exactly), JSON-only prompt, tolerant recovery from a fenced or prose-wrapped response, media type from MAGIC BYTES never the extension. **`extractTipPicks` NEVER writes** - `insertTipPicks` is a separate explicit call, so a preview surface can show what a save would store (invariant 9's shape) - and it returns `ok: false` with a reason rather than throwing, since a failed extraction is normal on bad signal. `raw_extraction` + `image_sha256` are kept even on success, so a misread number is diagnosable without re-running a paid call. Builds no ticket, stakes nothing, grades nothing. `BETSHEET_TIP_TEST_MODE=1` + `stubResponse` is the check-script-only bypass, mirroring `BETSHEET_LLM_TEST_MODE`. |
...
| `scripts/check-tip-extraction.js` | verification for TIPSHEET extraction, TWO PHASES. Phase 1 is hermetic and always runs (throwaway DB, logger redirected there too): migration 028's shape and refusals, explicit proof `cards` gained no TIPSHEET value and no tip_* column, every odds conversion re-checked through `morningLineToDecimal`, the blocking rank cases, and the stubbed extract -> validate -> persist -> read-back path incl. replace-on-re-extract and the reset naming `tip_picks`. **Phase 2 needs real screenshots and a key and SKIPS loudly rather than failing** - no golden can exist for "did the model read this picture correctly", so it PRINTS each extraction beside its source file to be eyeballed, asserting only the machine-checkable part. Samples go in gitignored `scratch/tip-samples/` (someone else's app, not ours to commit). |
...
npm run check-tip-extraction  # D166 + D169 (routes: preview writes nothing, save re-parses the ARCHIVE and ignores client-supplied picks, a correction is marked and keeps the FIRST extraction, a hand-typed broken ranking refused 422): TIPSHEET screenshot extraction - migration 028, the hyphen->slash odds conversion re-checked through morningLineToDecimal, absent odds omitted not fabricated, a broken ranking blocking, the stubbed round trip; phase 2 skips loudly without real screenshots in scratch/tip-samples/
```
...
| feature | state | notes |
| --- | --- | --- |
| Manual tip-pick entry: type the sheet instead of photographing it (D176) | merged | branch `tipsheet-manual-entry` - **user request**: screenshots are tedious and cost an API call per race, when a tip sheet is three horses and three ranks. A per-race dialog, horse rows x tipsheet columns, writing the IDENTICAL payload extraction produces. **The grid shape came from the user's own data**: the described one-tipsheet-dropdown-per-row could not record Karazest as TrackMaster #1 AND NumberFire #2, which both real screenshots showed - Kevin's call was columns per sheet. Extraction is untouched here and removed separately (D177), so the new path proved out before the old one went. Full record: DELIVERABLES.md D176. |
...
- **Newer models REJECT a non-default `temperature`; the default value is fine**
  ...
  **So LLM cards are NOT affected and never were**; the failure only appears
  ...
  D166's vision extraction tried to do and why this was found. `complete()` in
