...
export function gradeAndPersist(db, cardId, correlationId) {
  ...
  const save = db.transaction(() => {
    db.prepare(`DELETE FROM graded_tickets WHERE ticket_id IN
        (SELECT id FROM tickets WHERE card_id = ?)`).run(cardId);
    const ins = db.prepare(`INSERT INTO graded_tickets
        (ticket_id, outcome, returned_cents, pl_cents, details, correlation_id)
        VALUES (?, ?, ?, ?, ?, ?)`);
    for (const g of grades) {
      ins.run(g.ticket.id, g.outcome, g.returnedCents, g.plCents,
  ...
  traceLog.info('card_graded', {
    correlationId: card.correlation_id, cardId, gradedBy: correlationId, ...summary,
  });
  log.info('card_graded', { correlationId, cardId, raceDayId: card.race_day_id, ...summary });
  return { cardId, summary, grades };
}
...
gradingRouter.post('/cards/:id/grade', (req, res) => {
  ...
  res.status(201).json({ correlationId, summary: result.summary });
});
