...
| D07 | Consensus-fetch framework: fetcher interface, scheduling/refresh, fetch audit log, manual fallback path | 1 | `consensus-framework` | in review | registry + runner: robots.txt (disallowed = never requested), backoff after 3 straight failures, track/date-mismatch discard, refresh replaces picks, audit in DB + fetch-audit stream; manual paste w/ read-only preview; `npm run check-consensus` vs. stub sources |
