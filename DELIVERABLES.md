...
| D06 | Ingest UI: paste/upload, track/date/bankroll, parse-preview with correction, save | 1 | `ingest-ui` | in review | preview-before-save is invariant 9; warnings-first editable preview, 409+replace on duplicate day, migration 002 (`races.wager_menu`); `npm run check-ingest` boots the real server on a temp DB |
