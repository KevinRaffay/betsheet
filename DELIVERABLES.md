...
| D05 | Entries parser — program PDF (jumbled columns, index validation, handicapper-analysis extraction) | 1 | `pdf-parser` | in review | pdfjs-dist; panels anchored by date footer, bands by program-number y; real Del Mar program fixture (10 races, 98 entries); Bottom Line -> programRank + Best Bet; index cross-validation caught the printed-scratch/renumbered-index pattern; `npm run check-program` |
