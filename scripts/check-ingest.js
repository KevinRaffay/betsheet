...
try {
  ...
  // rather than listing them, so this assertion cannot go stale the same way.
  {
    ...
    check('after reset: strategy_templates holds exactly the five producers, reseeded (D436 added combined)',
      seeded.join(',') === 'combined,equibase-otr,human,llm,tipsheet', seeded.join(','));
