...
### Static-web cards imported from the Pages target (D153; HISTORICAL as of D236, which deleted `scripts/import-static-cards.js` along with the static app's card-construction feature - nothing emits this event any more, but the events already written under it remain valid history and are read exactly as before)

A card built on a phone at the track (D151) and carried home as a file (D152). It was imported through `persistHumanRace`, the same writer the desktop uses, so it emitted the **full human-card event set above** - `card_generated`, one `ticket_added` per ticket, one `human_race_locked` per race - plus the single event below. Every one of them carries the one correlationId that import minted for the card (invariant 8).

Two things about these events are worth knowing when reading a trace that has them. The `ts` on an imported `human_race_locked` is the IMPORT time, because that is when the event was written; the time the race was actually locked at the track is in `human_race_state.picks_locked_at`, which the import re-stamped from the file. And a re-import of an already-present card wrote nothing at all - not even this event - because the import skipped it entirely.

| event | fields | meaning |
| --- | --- | --- |
| `static_card_imported` | `cardId` (the local row id just minted), `raceDayId`, `externalId` (the device-namespaced card id, then on `cards.external_id` - the idempotency key), `deviceId`, `builtOn` (`'static-web'`), `payloadHash` (the payload the phone built against, verified to still reproduce from this database), `sawReferenceCards`, `races`, `exportedAt` (the file's own stamp) | one card arrived from outside this machine. The one place a card's origin was something other than a local action, which is why it was its own event rather than a field on `card_generated` |
...
| `llm_response_received` | `correlationId`, `cardId`, `raceDayId`, `responseChars`, `parsedTicketCount`, `parsedRaceCount` (0 on any hard failure, else 1), `parseErrors` (an API/network error message; `'no_ticket_block'` when the response had no parseable ticket block; else the `type` of every BLOCKING parse warning), `usage` (D420, on a clean parse only: `{input, output, cacheCreation, cacheRead}` as the API reported them - `input` is the UNCACHED remainder, the whole prompt is the sum of the three input figures; `null` for a stub or when the API sent no usage block; the same four figures sit on the `llm_card_requests` row) | the outcome of the same call, whatever it was - a hard failure, an unparseable response, or a clean parse still gets one of these |
