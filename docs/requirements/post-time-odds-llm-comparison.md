...
whose share of the book moved. It is deliberately a LABEL - it names the move
and says nothing about whether the move is worth backing, because the two
readings of a steamer ("informed money" and "the drifter is now the value")
are contradictory and this document is what is supposed to settle which one
pays, with an `n`. The one thing it changes for these phases is incidental and
useful: every board typed for PT-3/PT-4 is now also legible at the track, so
the collection step has a reason to happen beyond feeding a future analysis.
