...
npm run check-live-odds       # D224: the live odds capture - pure reconciler (track/date mismatch, a board that never came up, a race or horse on only one side, scratch drift REPORTED but never applied, an unreadable price kept as text with a null decimal, degenerate input refusing rather than throwing, every warning carrying a message) plus the real server on a temp DB: preview writes nothing but still audits, confirm writes prices only, a second capture APPENDS with the first board still readable, odds_captured_at never rewinds, a refusal is 422 with nothing written. The real 4.2MB Del Mar page must be REFUSED (saved before wagering) - the priced fixtures are synthetic and say so
