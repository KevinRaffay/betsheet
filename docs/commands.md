...
npm run allocate-deliverable -- --title "Short title" [--branch b] [--who name]  # D203: atomically claim the next D number - run this BEFORE naming a branch or writing a ledger row; also --list, --status <n>, --release <n> [--reason "..."]. D226: on a BRAND-NEW database (a fresh clone, a cloud session) the first number is seeded from DELIVERABLES.md's high-water mark, not a constant, and the seed it chose is printed to stderr; stdout stays exactly `D<number>`
npm run check-deliverable-numbers  # D203: the allocator's own verification, incl. 25 real concurrent processes claiming 25 distinct numbers; D226: ledger-seed parsing (ID column only, suffixed IDs, CRLF, missing file), the floor that a stale ledger can never lower, a live assertion that the REAL DELIVERABLES.md still parses, and 8 processes racing to CREATE one database agreeing on a single seed
