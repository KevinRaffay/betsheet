...
code - including that no track's timezone was stored or derivable anywhere in
this codebase before D209, which was the one design question the whole
feature turned on:
[docs/requirements/race-day-calendar.md](docs/requirements/race-day-calendar.md).
All three phases delivered: C-1 (per-track timezone data, **D209**), C-2 (a
read-only `/api/calendar` endpoint, **D210**), C-3 (the client matrix view,
**D211**).

| Requirement | Deliverables |
| --- | --- |
| A calendar view, reachable from a button on the race-day list, defaults to the browser's own current date | D211 ("Race calendar" button on `RaceDayList.jsx`; the date input defaults to today's PACIFIC date, computed explicitly - the 2026-09-10 decision the user is always Pacific) |
| A matrix of every track racing that date (rows) against 24 hourly columns starting 10:00 AM Pacific (columns), assuming one race day per track | D210 (the data), D211 (`RaceDayCalendar.jsx` renders it as a `grid grid--matrix` table) |
| Each populated cell shows the race number and its post time **converted to Pacific** (the user is always Pacific - 2026-09-10 decision), and is a hyperlink to that track's stored race day (`/day/:id`) - navigating to an individual race is out of scope, and no route exists for it | D210 (`postTimePacific` per race), D211 (the clickable cell, browser-verified navigating to `/day/:id`) |
| Placing AND printing a race in the Pacific-anchored grid requires converting its printed local post time from the track's own zone, which requires a per-track timezone - not stored on `races.post_time` (a zoneless printed string; the parser reads a zone off the page but `insertRaceDay` discards it) and, before D209, not derivable from `shared/track-codes.js`'s registry either | D209 (the registry's new `tz` field, individually verified per track rather than assumed from its state, plus the pure DST-safe conversion helper), D210 (wired into a real endpoint) |
| A race with no parseable post time, or a track with no known timezone, is never silently dropped from the grid - it is surfaced as a visible count, matching invariant 11's standing preference for a visible gap over a silent one | D210 (`unplaceable`, per track, in the endpoint's own response shape) |

## Post-time odds vs the morning line: does the LLM find an edge as odds move? (thesis evaluated 2026-09-11)

Full evaluation of a user thesis - *"the LLM bets generated at post time are
better than the bets placed with the M/L; all inputs the same, only the odds fed
to the LLM change"* -
[docs/requirements/post-time-odds-llm-comparison.md](docs/requirements/post-time-odds-llm-comparison.md).
**EVALUATED, NOT SCHEDULED**; five phases (PT-0 to PT-4) are proposed and none
claims a deliverable number until it is picked up. The verdict is that the thesis
is well-posed and not testable today for three independent reasons - the input
does not exist anywhere in the corpus, the ROI metric it implies cannot be
powered at this corpus size, and the comparison as stated is confounded toward a
false positive - and that a smaller, cheaper question (does the EXISTING
ML-only corpus beat the closing line?) is nearly answerable for zero API calls.

| Requirement | Deliverables |
| --- | --- |
| Post-time odds have to exist before anything can be fed them: `entries.live_odds` is filled on 0 of 13,710 rows (D171), the Equibase entries page's own LiveOdds column was empty in all 113 entries of the only verified capture, and BOTH Apify actors declare win odds structurally absent - so the default ingestion path cannot carry them at all | - (evaluated, not built; PT-2/PT-4) |
| `shared/chart-parser.js` ALREADY extracts per-starter post-time odds and the favorite marker (lines 57-68) and `saveResults` drops them - `race_results` has no column. A migration plus a passthrough in one INSERT recovers a real odds board for every EQUIBASE-CHART day - but only those: the pre-pivot archive's results are 72 `dmtc_html` to 3 `equibase_pdf`, and the DMTC parser extracts no odds at all, so those days need a different document fetched per day. **The query that decides whether this is cheap or nearly useless** - how many of the 7 live days with results came in as Equibase charts - could not be run from the evaluating session | - (evaluated, not built; PT-1) |
| A hit-rate win by a post-time arm is NOT evidence of edge - post-time odds are strictly better information than a morning line, so that arm should improve whether or not the model contributed anything. Edge requires ROI at real payouts or a beat-the-close measure, each read against the post-time favorite, the ML favorite and random on the SAME races | - (evaluated, not built; PT-1/PT-3) |
| ROI cannot be powered on this corpus: ~99 LLM races with results across 7 race days, against ~18,000 races for a 5-point ROI difference and a ~68-point minimum detectable effect at n=99. Beat-the-close needs ~590, and the one-sample "does the current corpus beat the close?" is nearly powered today | - (evaluated, not built; PT-1/PT-2) |
| Generation is irreducibly stochastic - `temperature: 1` by default and D166 found newer models answer 400 to `temperature: 0` - so within-arm self-agreement had to be measured before any one-generation-per-arm A/B could be believed. **ANSWERED FOR FREE, 2026-09-11**: `llm_card_requests` already stores repeated same-prompt generations, and the committed pre-pivot archive holds 7 such groups (20 generations) - primary win pick agrees on 24 of 25 pairs (96%) while every response text differs, so a pick change between arms is signal. Stake and ticket count are NOT that stable, so money-based metrics still want replicates | - (measured, not built; PT-0) |
