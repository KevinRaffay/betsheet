...
| `shared/chart-parser.js` ALREADY extracts per-starter post-time odds and the favorite marker (lines 57-68) and `saveResults` drops them - `race_results` has no column. A migration plus a passthrough in one INSERT recovers a real odds board for every chart-route day, and is the smallest useful change in the whole evaluation | - (evaluated, not built; PT-1) |
...
| Generation is irreducibly stochastic - `temperature: 1` by default and D166 found newer models answer 400 to `temperature: 0` - so within-arm self-agreement must be measured (k=5 on ~20 races, ~$2) before any one-generation-per-arm A/B is believed | - (evaluated, not built; PT-0) |
