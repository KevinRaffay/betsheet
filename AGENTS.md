...
| `scripts/dev-preview.js` | `npm run dev:preview`: the dev stack an AGENT starts, on ports that are never the human's (api 8795, vite 5185) - `.Codex/launch.json` points here instead of `dev`, so browser verification structurally cannot take 8788/5175. |
...
npm run dev:preview    # D97: the AGENT's dev stack, api :8795 / vite :5185, never the human's ports (.Codex/launch.json runs this)
...
- Feature branches off `main`, short kebab-case names, PR into `main`.
  **Codex opens AND merges its own PRs into `main`, without asking each
  ...
  which itself replaced "Codex never merges". The reason for the change is
  ...
  **Nothing else about the discipline relaxes, and one thing tightens: with
  ...
  it runs BEFORE every merge, not after.** Codex still never merges a red
...
2. `AGENTS.md` reflects any changed invariant, architecture entry, command
   or feature status.
...
| TIPSHEET: a third-party tip app's picks (D166; **screenshot path removed by D177 - picks are typed now, D176**) | merged | PR [#204](https://github.com/KevinRaffay/betsheet/pull/204), branch `Codex/tipsheet-screenshot-extraction-14fc5e` - a NEW `tip_picks` table (migration 028), not a widened `cards`: a tip row is a ranked opinion, so `consensus_completeness` gained no TIPSHEET value. **No completeness column, deliberately** - every tip row has one signal, its ranking. Archaeology found the conversion this turns on: `morningLineToDecimal` cannot read the hyphenated `9-2` tip apps actually print. Full record: DELIVERABLES.md D166. |
...
- **Ports**: BetSheet uses api :8788 / vite :5175 for the HUMAN's `npm run dev`
  ...
  pair - `.Codex/launch.json` already points there, so `preview_start` gets
  ...
  both in `.Codex/launch.json`. Neither has an API half - the static app has
...
- **Newer models REJECT a non-default `temperature`; the default value is fine**
  (D166). `Codex-sonnet-5` answers a 400 `\`temperature\` is deprecated for this
