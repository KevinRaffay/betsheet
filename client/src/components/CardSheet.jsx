...
//
// Extracted from CardView.jsx (D237) so a caller that already HAS card/grade
// data (rather than a cardId to fetch by) can render the identical sheet - a
// second renderer would be a second copy of the money, subtotal and
// grade-join logic, free to drift from the real card page. CardView.jsx
// composes this with its own fetches and, for the standalone page, its own
// pagehead (Export/Grade/Delete/Back); this component owns none of that.
//
// PROP-DRIVEN, not id-driven, and deliberately keyed on facts a payload can
// carry rather than raw database row ids:
//   * single-vs-multi-race grouping reads `t.selections.races` (every ticket
//     is written with `selections: {races, legs}`, server/human-cards.js),
//     not `t.race_id` - a SQLite foreign key with no meaning outside this
//     database.
//   * the ticket-to-grade join reads `t.sequence`/`g.sequence` (unique per
//     card, present on every ticket and every graded-ticket row), not
//     `t.id`/`g.ticket_id` - again a local row id.
//
// `card.llmModelLabel` is the PRE-RESOLVED display label, not the raw
// `llm_model` id - `../api.js`'s `modelLabel()` does that lookup today, and
// this component must never import api.js (a client-side fetch module) just
// to reach a pure string table; the caller resolves it and hands over the
// string.
export default function CardSheet({ card, races, results, notesByRace, grades }) {
  ...
  const summary = graded ? grades.summary : null;

  const singles = card.tickets.filter((t) => t.selections.races.length === 1);
  ...
  for (const r of results?.finishers ?? []) {
  ...
  for (const x of results?.exotics ?? []) resultsByRace.get(x.race_number)?.exotics.push(x);
  for (const s of results?.scratches ?? []) resultsByRace.get(s.race_number)?.scratches.push(s);
  ...
  return (
    <section className="card-sheet">
      <p className="dim">
        ...
        {card.llmModelLabel && <> · model <code>{card.llmModelLabel}</code></>}{card.notes_present ? <> · <span className="tag tag--gold">analyst notes</span></> : null} · bankroll {money(card.bankroll_cents)}
      ...
      {card.allocations.map((a) => {
        const race = (races ?? []).find((r) => r.number === a.race_number);
            ...
            {raceTickets.length === 0
              ...
              : (
                <table className="grid">
                  ...
                  <tbody>
                    {raceTickets.map((t) => (
                      <TicketRow key={t.sequence} t={t} g={gradeByTicket.get(t.sequence)} graded={graded} />
                    ))}
...
function GradeSubtotal({ tickets, gradeByTicket }) {
  const gs = tickets.map((t) => gradeByTicket.get(t.sequence)).filter(Boolean);
